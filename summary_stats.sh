...
do
        ...
        rawline=$(wc -l < data/raw_fastq/${rawsam}.fastq)
        rawread=$(echo "$rawline / 4" | bc)
        echo "$rawsam result: $rawread"
        echo "$rawsam,$rawread" >> summary_results/raw_read_count.csv
done
...
do
	...
	trimline=$(wc -l < data/trimmed_fastq/${trimsam}.fastq)
	trimread=$(echo "$trimline / 4" | bc)
	echo "$trimsam result: $trimread"
	echo "$trimsam,$trimread" >> summary_results/trimmed_read_count.csv
done
...
do
	...
	echo "$var,$count" >> summary_results/aligned_read_count.csv
done
...
do
	...
	echo "$three,$site" >> summary_results/variant_site_count.csv
done
